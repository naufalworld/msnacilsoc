...
function generateFullSchedule() {
    return {
        Monday: [
            { time: "08:00 - 09:30", activity: "Korean Language" },
            { time: "09:30 - 11:50", activity: "Physical" },
            { time: "12:20 - 13:40", activity: "Sociology" },
            { time: "13:40 - 15:00", activity: "Informatics" }
        ],
        Tuesday: [
            { time: "07:00 - 08:30", activity: "Art and Culture" },
            { time: "08:30 - 10:50", activity: "Korean Language" },
            { time: "10:40 - 12:10", activity: "Sundanese Language" },
            { time: "12:40 - 14:00", activity: "Mathematics" },
            { time: "14:00 - 15:20", activity: "History" }
        ],
        Wednesday: [
            { time: "07:00 - 09:10", activity: "Informatics" },
            { time: "09:00 - 11:30", activity: "Sociology" },
            { time: "11:30 - 13:20", activity: "Economics" },
            { time: "13:00 - 15:20", activity: "Geography" }
        ],
        Thursday: [
            { time: "07:00 - 09:10", activity: "Economics" },
            { time: "09:10 - 10:50", activity: "Geography" },
            { time: "10:50 - 12:10", activity: "Civics" },
            { time: "12:40 - 14:30", activity: "Indonesian Language" },
            { time: "14:00 - 15:20", activity: "Mathematics" }
        ],
        Friday: [
            { time: "08:00 - 09:20", activity: "Indonesian Language" },
            { time: "09:20 - 11:20", activity: "English Language " },
            { time: "12:30 - 14:30", activity: "Religious Studies " },
            { time: "14:30 - 15:10", activity: "Guidance and Conseling" },
...
function displaySchedule() {
    ...
    for (const day in schedule) {
        ...
        previewRow.classList.add('preview');
        
        const dayCell = document.createElement("td");
        dayCell.textContent = day;
        previewRow.appendChild(dayCell);
        
        const previewCell = document.createElement("td");
        previewCell.colSpan = 2;
        const firstClass = schedule[day][0];
        const lastClass = schedule[day][schedule[day].length - 1];
        const previewText = `${schedule[day].length} classes (${firstClass.time} - ${lastClass.time})`;
        previewCell.innerHTML = `
            <div class="preview-content">
                <span class="day-preview">${previewText}</span>
                <button class="read-more-btn" onclick="toggleDayDetails('${day}')">
                    <i class="fas fa-chevron-down"></i> View Schedule
                </button>
            </div>
        `;
        ...
        schedule[day].forEach((session, index) => {
            ...
            row.classList.add(`${day}-details`);
            row.style.display = 'none';
            
            if (index === 0) {
                const dayCell = document.createElement("td");
                dayCell.textContent = day;
                dayCell.rowSpan = schedule[day].length;
...
function isTimeInRange(current, start, end) {
    const curr = new Date(`2000/01/01 ${current}`);
    const st = new Date(`2000/01/01 ${start}`);
    const en = new Date(`2000/01/01 ${end}`);
...
function toggleDayDetails(day) {
    const detailRows = document.querySelectorAll(`.${day}-details`);
    const button = document.querySelector(`tr.preview button`);
